...
struct HashTable{
    ...
    void hash_lookup(string user_name){
        ...
        else if(password[hash] == user_name){ 
            cout<<"Password for user "<<user_name<<" is "<<password[hash]<<"\n";
        }
        else{
            int i = (hash + 1) % MAX_LENGTH;
            while(i != hash && (is_slot_empty(i) || password[i] != user_name)){
                i = (i + 1) % MAX_LENGTH;
            }
            if(i == hash || is_slot_empty(i)){
                cout<<"No item found\n";
            }
            else{
                cout<<"Password for user "<<user_name<<" is "<<password[i]<<"\n";
            }
        }
...
int main(){
    // added this first line for the convenience
    cout<< "1. Insert\n2. Delete item\n3. Hash lookup password\n4. Print Hashtable\n-1.Exit\n";
